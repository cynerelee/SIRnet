    ...
    def eval_model_new(self):
        ...
        with torch.no_grad():
            ...
            for data in tqdm(self.test_data):
                ...
                mask=mask.to(device)
                loss,outputs= self.model(img,GT,instrumentList,verbList,targetList,instrumentGT,verbGT,targetGT,mask)
        ...
        return result,avg_loss.avg,mAP,AP
    def train_model(self):
        ...
        for epoch in range(start_epoch,self.args.max_epoch):
           ...
           # lr_list.append(self.optimizer.state_dict()['param_groups'][0]['lr'])
            ...
            result,testloss,mAP,AP = self.eval_model_new()
       ...
       #     self.saveToTxt(int(epoch), result,
        #                   './checkpoints/test_loss.txt')
            ...
            if f1 > best_f1:
                print("Achieving Best f1 on Test Set.")
                print("f1 on Test:",f1)
                # checkpoint_dict = {'epoch': epoch,
                #                    'model_state_dict': self.model.state_dict(),
                #                    'best_result':f1
                ...
                torch.save(self.model.state_dict(),'%s/best/bestF1.pth'% (self.generated_param_directory))
                #torch.save({'state_dict': self.model.state_dict()}, self.generated_param_directory + " epoch_%d_f1_%.4f.model" %(epoch, result))
                best_f1 = f1
                #best_f1_epoch = epoch
                #np.save('E:\\jianguoyun\\n32\\Abstudy\\BCEloss\\Absstudy\\our\\checkpoints\\best\\bestAP.npy',AP)
            
           
            if mAP > best_mAP:
                print("Achieving Best mAP on Test Set.")
                print("mAP on Test:",mAP)
                # checkpoint_dict = {'epoch': epoch,
                #                    'model_state_dict': self.model.state_dict(),
                #                    'best_result':f1
                #                   # 'optim_state_dict': self.optimizer.state_dict()
                ...
                # torch.save(checkpoint_dict,
                #            '%s/best/epoch_%d_%.4f.pth.tar' % (self.generated_param_directory, epoch, f1))
                torch.save(self.model.state_dict(),'%s/best/bestmAP.pth'% (self.generated_param_directory))
                #torch.save({'state_dict': self.model.state_dict()}, self.generated_param_directory + " epoch_%d_f1_%.4f.model" %(epoch, result))
                best_mAP = mAP
